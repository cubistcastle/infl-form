    ...
    if ($.fn.inputmask == undefined) {
        ...
        $.fn.inputmask = function(fn, options) {
            ...
            if (android) {
                ...
                var version = parseInt(new RegExp(/[0-9]+/)
                    .exec(browser));
                android = version <= 533;
            }
            ...
            function getMaskTemplate(mask) {
                var escaped = false,
                    outCount = 0;
                if (mask.length == 1 && opts.greedy == false) {
                    opts.placeholder = "";
            ...
            function generateMaskSets() { //TODO improve generate masksets
                ...
                function generateMask(maskPrefix, maskPart) {
                    ...
                    if (masks.length > 1) {
                        ...
                        if (maskParts.length > 1 && maskParts[1].split(opts.optionalmarker.start)
                            .length > 1) {
                ...
                $.each(masksets, function(index, value) {
                    ...
                    if (isRTL ? activeMaskset['lastValidPosition'] <= seekNext(buffer, maskPos) : activeMaskset['lastValidPosition'] >= seekPrevious(buffer, maskPos)) {
                        if (maskPos >= 0 && maskPos < getMaskLength()) {
                            var testPos = determineTestPosition(maskPos),
                                loopend = c ? 1 : 0,
                                chrs = '';
                ...
                $.each(masksets, function(index, value) {
                    ...
                    if (isRTL ? activeMaskset['lastValidPosition'] <= pos : activeMaskset['lastValidPosition'] >= pos) {
                        ...
                        if (activeMasksetIndex != currentActiveMasksetIndex) {
                            var abl = getMaskLength(),
                                bufTemplate = getActiveBuffer();
                            if (isRTL) {
                                buffer.reverse();
                                bufTemplate.reverse();
                            }
            ...
            function getMaskLength() {
                var calculatedLength = getActiveBuffer()
                    .length;
                if (!opts.greedy && opts.repeat > 1) {
                    calculatedLength += (getActiveBuffer()
                        .length * (opts.repeat - 1));
                }
            ...
            function clearBuffer(buffer, start, end) {
                for (var i = start, maskL = getMaskLength(); i < end && i < maskL; i++) {
                    setBufferElement(buffer, i, getBufferElement(getActiveBuffer()
                        .slice(), i));
                }
            };
            ...
            function checkVal(input, buffer, clearInvalid, skipRadixHandling) {
                ...
                buffer.length = getActiveBuffer()
                    .length;
                var lastMatch = -1,
                    checkPosition = -1,
                    np, maskL = getMaskLength(),
                ...
                if (opts.greedy == false) {
                    var newBuffer = truncateInput(buffer.join(''), isRTL)
                        .split('');
            ...
            function truncateInput(inputValue, rtl) {
                return rtl ? inputValue.replace(new RegExp("^(" + escapeRegex(getActiveBuffer()
                    .join('')) + ")*"), "") : inputValue.replace(new RegExp("(" + escapeRegex(getActiveBuffer()
                    .join('')) + ")*$"), "");
            }
                ...
                if ($(input)
                    ...
                    for (var pos = 0; pos <= tmpBuffer.length - 1; pos++) {
                        ...
                        if (getActiveTests()[testPos].optionality) {
                            if (getPlaceHolder(pos) == buffer[pos] || !isMask(pos)) tmpBuffer.splice(0, 1);
                            else break;
                        } else break;
                    }
                } else {
                    for (var pos = tmpBuffer.length - 1; pos >= 0; pos--) {
                        var testPos = determineTestPosition(pos);
                        if (getActiveTests()[testPos].optionality) {
                            if (getPlaceHolder(pos) == buffer[pos] || !isMask(pos)) tmpBuffer.pop();
                            else break;
                        } else break;
                    }
                }
            ...
            function unmaskedvalue($input, skipDatepickerCheck) {
                ...
                if (getActiveTests() && (skipDatepickerCheck === true || !$input.hasClass('hasDatepicker'))) {
                    ...
                    return $.map(buffer, function(element, index) {
                        return isMask(index) && element != getBufferElement(getActiveBuffer()
                            .slice(), index) ? element : null;
                    })
                        .join('');
                } else {
            ...
            function mask(el) {
                ...
                if (getMaskLength() > maxLength && maxLength > -1) { //FF sets no defined max length to -1 
                    if (maxLength < getActiveBuffer()
                        .length) getActiveBuffer()
                        .length = maxLength;
                    if (opts.greedy == false) {
                        opts.repeat = Math.round(maxLength / getActiveBuffer()
                            .length);
                    }
                ...
                $input.bind("mouseenter.inputmask", function() {
                    ...
                    if (!$input.hasClass('focus.inputmask') && opts.showMaskOnHover) {
                        var nptL = input._valueGet()
                            .length;
                        if (nptL < buffer.length) {
                            if (nptL == 0) buffer = getActiveBuffer()
                                .slice();
                            writeBuffer(input, buffer);
                        }
                    }
                    ...
                    if (opts.clearMaskOnLostFocus && nptValue != '') {
                        if (nptValue == getActiveBuffer()
                            .join('')) input._valueSet('');
                    ...
                    if (opts.clearMaskOnLostFocus) {
                        if (!$input.hasClass('focus.inputmask')) {
                            if (input._valueGet() == getActiveBuffer()
                                .join('') || input._valueGet() == '') input._valueSet('');
                            else { //clearout optional tail of the mask
                                clearOptionalTail(input, buffer);
                            }
                        }
                    ...
                    setTimeout(function() {
                        ...
                        if (selectedCaret.begin == selectedCaret.end) {
                            ...
                            if (isRTL) caret(input, clickPosition > lastPosition && (isValid(clickPosition, buffer[clickPosition], buffer, true, isRTL) !== false || !isMask(clickPosition)) ? clickPosition : lastPosition);
                            else caret(input, clickPosition < lastPosition && (isValid(clickPosition, buffer[clickPosition], buffer, true, isRTL) !== false || !isMask(clickPosition)) ? clickPosition : lastPosition);
                        }
                    }, 0);
                })
                ...
                function installEventRuler(npt) {
                    var events = $._data(npt)
                        .events;

                    $.each(events, function(eventType, eventHandlers) {
                        $(npt)
                            .bind(eventType + ".inputmask", function(event) {
                            ...
                            $.fn.val = function() {
                                if (arguments.length == 0) {
                                    ...
                                    if ($self.data('inputmask')) {
                                        if ($self.data('inputmask')['autoUnmask']) return $self.inputmask('unmaskedvalue');
                                        else {
                ...
                function shiftL(start, end, c) {
                    ...
                    buffer = truncateInput(buffer.join(''), isRTL)
                        .split('');
                    if (buffer.length == 0) buffer = getActiveBuffer()
                        .slice();
                    ...
                    for (var i = start; i <= end && i < getMaskLength(); i++) {
                        if (isMask(i)) {
                            ...
                            if (t != getPlaceHolder(i)) {
                                var j = seekNext(buffer, i);
                                if (j < getMaskLength()) {
                                    if (isValid(j, t, buffer, true, isRTL) !== false && getActiveTests()[determineTestPosition(i)].def == getActiveTests()[determineTestPosition(j)].def) c = t;
                                    else {
                                        if (isMask(j)) break;
                                        else c = t;
                                    }
                                ...
                                if ((np = isValid(p, c, buffer, false, isRTL)) !== false) {
                                    ...
                                    if (refresh !== true) {
                                        if (opts.insertMode == true) {
                                            ...
                                            if (lastUnmaskedPosition >= p) shiftR(p, buffer.length, c);
                                            else return false;
                ...
                function keyupEvent(e) {
                    ...
                    if (k == opts.keyCode.TAB && $input.hasClass('focus.inputmask') && input._valueGet()
                        .length == 0) {
                        buffer = getActiveBuffer()
                            .slice();
                        writeBuffer(input, buffer);
